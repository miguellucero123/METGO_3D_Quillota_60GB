"""
DASHBOARD METEOROLÓGICO METGO 3D QUILLOTA
Dashboard especializado en monitoreo climático en tiempo real
"""
...
class DashboardMeteorologico:
    ...
    def inicializar_db(self):
        """Inicializar base de datos meteorológica"""
        try:
    ...
    def obtener_datos_reales(self):
        """Obtener datos meteorológicos reales desde APIs"""
        try:
            with st.spinner("🔄 Obteniendo datos meteorológicos reales..."):
                ...
                if datos_obtenidos and 'datos_combinados' in datos_obtenidos:
                    ...
                    for _, fila in df_combinado.iterrows():
                        ...
                        indice_uv = fila.get('radiacion_solar', None)  # Usar radiación solar como proxy para UV
                        
                        # Obtener el nombre de la estación
                        estacion = "Estación Desconocida"
                        if 'estacion' in df_combinado.columns:
                            estacion = fila.get('estacion', 'Estación Desconocida')
                        elif 'station' in df_combinado.columns:
                            estacion = fila.get('station', 'Estación Desconocida')
                    ...
                    st.success(f"✅ Datos reales obtenidos de {total_estaciones} estaciones")
                    return True
                else:
                    st.warning("⚠️ No se pudieron obtener datos reales, generando datos simulados...")
                    return self.generar_datos_simulados()
                    
        except Exception as e:
            st.error(f"Error obteniendo datos reales: {e}")
            st.info("🔄 Intentando con datos simulados...")
            return self.generar_datos_simulados()
    
    def generar_datos_simulados(self):
        """Generar datos meteorológicos simulados para demostración"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            conn.commit()
            conn.close()
            st.success("✅ Datos meteorológicos simulados generados")
            return True
    ...
    def mostrar_debug_datos(self):
        """Mostrar información de debug sobre los datos obtenidos"""
        try:
            with st.spinner("🔍 Analizando estructura de datos..."):
                ...
                if datos_obtenidos and 'datos_combinados' in datos_obtenidos:
                    ...
                    st.subheader("🔍 Información de Debug - Estructura de Datos")
                    ...
                    with col2:
                        ...
                        if len(df) > 0:
                            ...
                            st.write("**Valores únicos en columnas principales:**")
    ...
    def obtener_datos_actuales(self):
        ...
        try:
            ...
            # Datos más recientes por estación
            query = '''
                SELECT estacion, 
    ...
    def obtener_datos_historicos(self, horas=24):
        ...
        try:
            ...
            # Obtener todos los datos y filtrar después en pandas
            query = '''
            ...
            if not df.empty:
                ...
                # Filtrar por las últimas N horas si hay datos
...
def main():
    """Función principal del dashboard meteorológico"""
    st.set_page_config(
    ...
    # Header
    st.title("🌤️ Dashboard Meteorológico METGO 3D")
    st.markdown("**Monitoreo Climático en Tiempo Real - Valle de Quillota**")
    ...
    with st.sidebar:
        ...
        with col1:
            if st.button("🌐 Datos Reales"):
                dashboard.obtener_datos_reales()
                st.rerun()
        
        with col2:
            if st.button("🧪 Datos Simulados"):
        ...
        periodo = st.selectbox(
            "📅 Período de Análisis",
            ["Últimas 6 horas", "Últimas 12 horas", "Últimas 24 horas", "Últimos 3 días"]
        )
        ...
        estacion_seleccionada = st.selectbox("📍 Estación", estaciones)
    ...
    if datos_actuales.empty:
        st.warning("⚠️ No hay datos meteorológicos disponibles. Genera datos de prueba desde el sidebar.")
        return
    ...
    if not datos_actuales.empty:
        ...
        with col3:
            ...
            st.metric("🔽 Presión Promedio", f"{presion_promedio:.1f} hPa")
        
        with col4:
            viento_promedio = datos_actuales['velocidad_viento'].mean()
            st.metric("💨 Viento Promedio", f"{viento_promedio:.1f} km/h")
    ...
    if not datos_historicos.empty:
        ...
        try:
            ...
            # Intentar con formato más flexible
        ...
        fig_precip = px.bar(
            ...
            y='precipitacion', 
            color='estacion',
            title='🌧️ Precipitación por Estación',
        ...
        # Mapa de calor de variables meteorológicas
        st.subheader("🗺️ Mapa de Calor - Variables Meteorológicas")
        ...
        fig_heatmap = px.imshow(
            datos_heatmap.T,
            labels=dict(x="Estación", y="Variable", color="Valor"),
            title="🌡️ Mapa de Calor - Promedios por Estación",
            aspect="auto"
        )
    ...
    if not datos_actuales.empty:
        ...
        st.markdown("**Datos Más Recientes por Estación:**")
        st.dataframe(
    ...
    with st.expander("ℹ️ Información del Sistema"):
        st.markdown("""
        **Dashboard Meteorológico METGO 3D**
        
        - **Ubicación:** Valle de Quillota, Chile
        - **Estaciones Monitoreadas:** 6 estaciones meteorológicas
        - **Variables:** Temperatura, Humedad, Presión, Precipitación, Viento, Nubosidad, Índice UV
    ...
    st.markdown("---")
    st.markdown(
        "**METGO 3D Quillota** | Dashboard Meteorológico | "
