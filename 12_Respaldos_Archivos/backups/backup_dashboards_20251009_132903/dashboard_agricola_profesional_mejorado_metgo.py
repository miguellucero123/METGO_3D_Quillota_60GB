...
# -*- coding: utf-8 -*-
"""
DASHBOARD AGRÍCOLA PROFESIONAL MEJORADO - METGO 3D QUILLOTA
Sistema ultra sofisticado con gráficos de líneas profesionales y espaciado optimizado
"""
...
st.set_page_config(
    page_title="METGO 3D - Dashboard Agrícola Profesional",
    page_icon="🌱",
    layout="wide",
...
class SistemaVisualizacionProfesional:
    """Sistema de visualización profesional con gráficos de líneas optimizados"""
    ...
    def crear_grafico_temperaturas_lineas(self, datos_estaciones: Dict, datos_historicos: List[Dict]) -> go.Figure:
        """Crea gráfico de temperaturas con líneas profesionales"""
        if not datos_estaciones or not datos_historicos:
            return go.Figure()
        
        # Preparar datos históricos para líneas
        ...
        fig.update_layout(
            title=dict(
                text="🌡️ Evolución de Temperaturas por Estación",
                x=0.5,
                font=dict(size=24, color=self.colores_profesionales['texto'], showlegend=False)
            ),
            xaxis=dict(
                title="Hora del Día",
    ...
    def crear_grafico_humedad_lineas(self, datos_estaciones: Dict, datos_historicos: List[Dict]) -> go.Figure:
        """Crea gráfico de humedad con líneas profesionales"""
        if not datos_estaciones or not datos_historicos:
            return go.Figure()
        ...
        fig.update_layout(
            title=dict(
                text="💧 Evolución de Humedad Relativa por Estación",
                x=0.5,
                font=dict(size=24, color=self.colores_profesionales['texto'], showlegend=False)
            ),
            xaxis=dict(
    ...
    def crear_dashboard_estaciones_actuales(self, datos_estaciones: Dict) -> go.Figure:
        ...
        fig = make_subplots(
            ...
            subplot_titles=(
                ...
                'Presión Atmosférica por Estación',
        ...
        fig.update_layout(
            title=dict(
                text="📊 Estado Actual de Estaciones Meteorológicas",
                x=0.5,
                font=dict(size=26, color=self.colores_profesionales['texto'], showlegend=False)
            ),
            showlegend=False,
            ...
            plot_bgcolor='white',
            paper_bgcolor='white',
            margin=dict(l=60, r=60, t=120, b=60),  # Mejores márgenes
            annotations=[
                dict(
                    x=0.25, y=1.08,
                    xref='paper', yref='paper',
                    text='Temperatura Actual por Estación',
                    showarrow=False,
                ...
                dict(
                    x=0.75, y=1.08,
                    xref='paper', yref='paper',
                    text='Humedad Relativa por Estación',
                    showarrow=False,
                    font=dict(size=16, color=self.colores_profesionales['texto'])
                ),
                dict(
                    x=0.25, y=0.46,
                    xref='paper', yref='paper',
                    text='Presión Atmosférica por Estación',
                    showarrow=False,
                    font=dict(size=16, color=self.colores_profesionales['texto'])
                ),
                dict(
                    x=0.75, y=0.46,
                    xref='paper', yref='paper',
                    text='Velocidad del Viento por Estación',
                    showarrow=False,
                    font=dict(size=16, color=self.colores_profesionales['texto'])
                )
            ]
    ...
    def crear_analisis_comparativo(self, datos_estaciones: Dict) -> go.Figure:
        """Crea análisis comparativo con mejor presentación"""
        if not datos_estaciones:
            return go.Figure()
        ...
        for i, (estacion, datos) in enumerate(zip(estaciones, datos_completos)):
            ...
            hum_norm = datos['humedad']  # Ya está en 0-100
            ...
            fig.add_trace(go.Scatterpolar(
                ...
                theta=['Temperatura', 'Humedad', 'Presión', 'Viento'],
                fill='toself',
                name=estacion,
                line=dict(width=3),
                marker=dict(size=8)
        ...
        fig.update_layout(
            polar=dict(
                radialaxis=dict(
                    visible=True,
                    range=[0, 100],
                    tickfont=dict(size=12, showlegend=False),
            ...
            title=dict(
                text="🎯 Análisis Comparativo de Estaciones",
                x=0.5,
...
class DashboardAgricolaProfesionalMejorado:
    """Dashboard agrícola profesional con mejoras visuales"""
    ...
    def ejecutar(self):
        ...
        <div style="text-align: center; padding: 20px; background: linear-gradient(90deg, #2ecc71, #3498db); 
            ...
            <h1 style="color: white; margin: 0; font-size: 2.5em;">🌱 METGO 3D</h1>
            <h2 style="color: white; margin: 10px 0 0 0; font-size: 1.5em;">Dashboard Agrícola Profesional</h2>
            <p style="color: white; margin: 10px 0 0 0; font-size: 1.1em;">Sistema de Monitoreo Meteorológico Avanzado</p>
        </div>
        ...
        with st.sidebar:
            ...
            if st.session_state.ultima_actualizacion:
                st.success(f"✅ Última actualización: {st.session_state.ultima_actualizacion.strftime('%H:%M:%S')}")
                st.info(f"📈 Registros históricos: {len(st.session_state.datos_historicos)}")
            else:
        ...
        if st.session_state.datos_reales_apis:
            # Sección 1: Gráficos de líneas temporales
            st.markdown("## 📈 Análisis Temporal con Gráficos de Líneas")
            
            col1, col2 = st.columns(2)
            
            with col1:
                st.markdown("### 🌡️ Temperaturas por Estación")
                ...
                if fig_temp.data:
                    ...
                    st.info("📊 Se requieren más datos históricos para mostrar tendencias")
            
            with col2:
                st.markdown("### 💧 Humedad por Estación")
            ...
            # Sección 2: Dashboard actual con mejor espaciado
            st.markdown("## 📊 Estado Actual de Estaciones")
            ...
            if fig_dashboard.data:
                st.plotly_chart(fig_dashboard, config=PLOTLY_CONFIG, width='stretch')
            ...
            with col4:
                st.metric(
                    ...
                    delta=f"Máx: {np.max(vientos):.1f} km/h"
                )
            ...
            <div style="text-align: center; padding: 40px; background-color: #f8f9fa; 
                ...
                <h3 style="color: #2c3e50;">🚀 Dashboard Agrícola Profesional</h3>
                <p style="color: #7f8c8d; font-size: 1.2em; margin: 20px 0;">
                    Sistema avanzado de monitoreo meteorológico con gráficos de líneas profesionales
                </p>
                <div style="margin: 30px 0;">
                    <h4 style="color: #2c3e50;">✨ Características Principales:</h4>
                    <ul style="text-align: left; display: inline-block; color: #7f8c8d;">
                        <li>📈 Gráficos de líneas temporales para temperaturas y humedad</li>
                        <li>📊 Dashboard actual con espaciado optimizado</li>
                        <li>🎯 Análisis comparativo con gráficos de radar</li>
                ...
                <p style="color: #e74c3c; font-weight: bold; margin-top: 30px;">
                    👆 Use el botón "Actualizar Datos" en la barra lateral para comenzar
                </p>
            </div>
            """, unsafe_allow_html=True)
