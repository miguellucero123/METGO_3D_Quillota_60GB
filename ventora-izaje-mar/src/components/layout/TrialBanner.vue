...
const planCode = ref('')
const cuentaTo = computed(() =>
  faena.value ? `/f/${faena.value}/cuenta` : '/cuenta',
)
...
onMounted(async () => {
  if (!auth.isAuthenticated || !faena.value) return
  try {
    const data = await fetchCuenta(faena.value)
...
<template>
  <div v-if="visible" class="trial-banner" role="status">
    ...
    <RouterLink class="trial-banner__link" :to="cuentaTo">Ver planes</RouterLink>
  </div>
</template>
