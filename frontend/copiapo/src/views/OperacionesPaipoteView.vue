<template>
  <div class="page">
    <header class="page-head">
      <h1>Operaciones Paipote</h1>
      <p>
        Ventilación atmosférica faena · códigos
    ...
    <div class="actions">
      <a class="btn" :href="urlInforme('html')" target="_blank" rel="noopener">Informe HTML</a>
      <a class="btn btn-secondary" :href="urlInforme('pdf')" target="_blank" rel="noopener">Descargar PDF</a>
      <button type="button" class="btn btn-ghost" @click="cargar" :disabled="loading">Actualizar</button>
    </div>

    <div v-if="loading" class="state">Calculando ventilación Paipote…</div>
...
import { fetchPaipotePaquete, getApiBase } from '@/services/aireApi'
...
function urlInforme(formato) {
  return `${getApiBase()}/public/operaciones/paipote/informe?formato=${formato}`
}
...
async function cargar() {
  ...
  error.value = null
  try {
    pkg.value = await fetchPaipotePaquete()
  } catch (e) {
    error.value = e?.message || 'No se pudo cargar ventilación Paipote'
    pkg.value = null
  } finally {
    loading.value = false
  }
