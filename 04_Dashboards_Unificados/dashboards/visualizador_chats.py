...
class VisualizadorChats:
    ...
    def mostrar_contenido_archivo(self, archivo):
        ...
        try:
            ...
            print("\n🔧 OPCIONES:")
            print("1. Ver más líneas")
            print("2. Buscar texto específico")
    ...
    def ver_mas_lineas(self, contenido):
        ...
        try:
            inicio = int(input(f"Ingrese línea de inicio (1-{len(lineas)}): ")) - 1
            fin = int(input(f"Ingrese línea final ({inicio+1}-{len(lineas)}): "))
    ...
    def buscar_texto_en_archivo(self, contenido, nombre_archivo):
        ...
        if coincidencias:
            ...
            if len(coincidencias) > 20:
                print(f"... y {len(coincidencias) - 20} coincidencias más")
        else:
    ...
    def extraer_secciones(self, contenido, nombre_archivo):
        ...
        secciones = {
            'migración': ['migraci', 'migrar', 'migration'],
            'errores': ['error', 'fallo', 'problema', 'issue'],
            'mejoras': ['mejora', 'optimiz', 'mejorar', 'upgrade'],
            'instalación': ['instalar', 'setup', 'configurar', 'instalacion'],
        ...
        try:
            seleccion = int(input("\nSeleccione sección a extraer: ")) - 1
    ...
    def extraer_por_terminos(self, contenido, nombre_seccion, terminos, nombre_archivo):
        ...
        if lineas_extraidas:
            print(f"\n📄 SECCIÓN '{nombre_seccion.upper()}' EXTRAÍDA:")
            print("-"*50)
            ...
            if len(lineas_extraidas) > 30:
                print(f"\n... y {len(lineas_extraidas) - 30} líneas más")
            
            # Guardar extracción
            guardar = input("\n¿Guardar extracción en archivo? (s/n): ").strip().lower()
            if guardar == 's':
                self.guardar_extraccion(lineas_extraidas, nombre_seccion, nombre_archivo)
        else:
            print(f"❌ No se encontraron líneas para la sección '{nombre_seccion}'")
    ...
    def guardar_extraccion(self, lineas_extraidas, nombre_seccion, nombre_archivo):
        ...
        try:
            with open(nombre_archivo_salida, 'w', encoding='utf-8') as f:
                f.write(f"EXTRACCIÓN: {nombre_seccion.upper()}\n")
            ...
            print(f"✅ Extracción guardada en: {nombre_archivo_salida}")
        except Exception as e:
            print(f"❌ Error guardando extracción: {e}")
    ...
    def buscar_en_chats(self):
        ...
        for archivo in self.archivos_chat:
            try:
                ...
                print(f"⚠️ Error leyendo {archivo['nombre']}: {e}")
        ...
        if resultados:
            ...
            for resultado in resultados:
                ...
                if len(resultado['coincidencias']) > 5:
                    print(f"   ... y {len(resultado['coincidencias']) - 5} más")
        else:
            print("❌ No se encontraron coincidencias en ningún archivo")
    ...
    def extraer_conversaciones(self):
        ...
        print("4. Instalación y configuración")
        print("5. APIs y conectores")
        print("6. Personalizado")
        
        try:
            opcion = int(input("\nSeleccione tipo de conversación: "))
            
            if opcion == 1:
                self.extraer_por_tipo('migración', ['migraci', 'migrar', 'migration', 'disco', 'nube'])
            elif opcion == 2:
                ...
                self.extraer_por_tipo('instalación', ['instalar', 'setup', 'configurar', 'instalacion', 'dependencias'])
            elif opcion == 5:
                self.extraer_por_tipo('APIs', ['api', 'endpoint', 'request', 'response', 'conector'])
            elif opcion == 6:
                terminos = input("Ingrese términos separados por coma: ").split(',')
    ...
    def extraer_por_tipo(self, tipo, terminos):
        ...
        for archivo in self.archivos_chat:
            try:
                ...
                print(f"⚠️ Error procesando {archivo['nombre']}: {e}")
        
        if todas_las_lineas:
            print(f"✅ Encontradas {len(todas_las_lineas)} líneas relacionadas con '{tipo}'")
            ...
            if len(todas_las_lineas) > 20:
                print(f"\n... y {len(todas_las_lineas) - 20} líneas más")
            
            # Guardar extracción
            guardar = input(f"\n¿Guardar extracción de '{tipo}'? (s/n): ").strip().lower()
            if guardar == 's':
                self.guardar_extraccion_completa(todas_las_lineas, tipo)
        else:
            print(f"❌ No se encontraron líneas relacionadas con '{tipo}'")
    ...
    def guardar_extraccion_completa(self, lineas, tipo):
        ...
        try:
            with open(nombre_archivo, 'w', encoding='utf-8') as f:
                f.write(f"CONVERSACIONES EXTRAÍDAS: {tipo.upper()}\n")
                f.write(f"FECHA: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            ...
            print(f"✅ Extracción guardada en: {nombre_archivo}")
        except Exception as e:
            print(f"❌ Error guardando extracción: {e}")
    ...
    def crear_resumen(self):
        ...
        try:
            ...
            print(f"   📦 Tamaño total: {resumen['estadisticas']['tamaño_total_mb']:.2f} MB")
            print(f"   📝 Líneas totales: {resumen['estadisticas']['lineas_totales']:,}")
    ...
    def exportar_chat(self):
        ...
        try:
            ...
            if 0 <= seleccion < len(self.archivos_chat):
                ...
                print("4. Markdown (formato documentación)")
...
<body>
    <div class="header">
        ...
        <p>Tamaño original: {archivo['tamaño_mb']:.2f} MB</p>
    </div>
...
**Tamaño original:** {archivo['tamaño_mb']:.2f} MB  
...
---
*Exportado automáticamente por el Visualizador de Chats METGO 3D*
"""
                with open(nombre_archivo, 'w', encoding='utf-8') as f:
...
def main():
    ...
    print("Sistema METGO 3D - Versión 2.0")
